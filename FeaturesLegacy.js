// DEV-ONLY: frozen FeatureSet v1 snapshot for lastHash era 1 reproduction.
// Source: git tag features-calc-v1-submission @ commit a67f45c
// Excluded from submission bundle. Refresh only via explicit re-export + doc update.
// Includes shapeInterpreter PRNG draw (required for era-1 parity).
// Wrapped in IIFE so publicOptions / LilVert do not collide with Features.js.
  ...
  #initFeatureSets() {
    ...
    Object.keys(this.#options).forEach((optionKey) => {
      const
        ...
        enumFeature = new EnumFeatureLegacy(name, options, this.usageStore)
...
function normalizeSubRange(subrange, range) { return [normalize(subrange[0], range), normalize(subrange[1], range)] }
  ...
  window.FeatureSetLegacy = FeatureSetLegacy
})()