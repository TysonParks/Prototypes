...
| 12a | **Fix SVG filter/mask cropping** — cascade, r-out, waves+ordinal | Agent + verify | 12 | 🟡 In progress | Cascade cropping (§ 9.14.1) ✅ fixed — `boundsRect` override for frame layers. R-profile mask (§ 9.14.2) ✅ fixed — `maskUnits=userSpaceOnUse` with explicit bounds. Waves+ordinal (§ 9.14.3) remains. |
