...
| 12a | **Fix SVG filter/mask cropping** — cascade, r-out, waves+ordinal | Agent + verify | 12 | ✅ Combo cascade fixed | FrontGrid combo cascade/wave crop (§ 9.14.1) ✅ fixed June 2026 — `maskRect` + final `<mask>` at `FRAME` bounds in `createMaskGroup()`. Deferred: per-ShapeGroup tight mask sizing (Safari perf). R-profile mask (§ 9.14.2) ✅. Frame/backgrid `rIn` bbox (§ 9.14.11) ✅. Waves+ordinal (§ 9.14.3) deferred. |
