...
(currently `1` user unit — **pre-launch tune required**; see TODO in file for
possible split into `minCutDepth` vs `minInsetSpacing`).
...
| FC4 | **Color management: Display-P3 / Rec.2020 default in browsers.** Browsers may default to wide-gamut color profiles, slightly shifting how our `oklch()` colors render. | sRGB assumed; OKLCH helpers archived in `archive/color-experiments.js`. | If colors visibly drift, that's a platform-level change and our values were approximate anyway — collectors viewing on a wide-gamut display will see *more accurate* colors than we currently render in sRGB. Not a regression. |
