...
//TODO: DEPRECATE
// MARK: OLD CSS Shadow Functions
//#region OLD CSS Shadow Functions
// FUNC: shadowVector()
//shadowVector: create vector from Angle + Offset
// function shadowVector(angle = 45, offset = 8) {
//   return createVector(1, 0).rotate(angle).mult(offset)
// }

//TODO: DEPRECATE
// FUNC: boxShadowCSS()
// Box-Shadow CSS
function boxShadowCSS(x, y, blurRad = 0, spreadRad = 0, col = color(0), inset = false) {
  let color = col.toString('#rrggbb')
  if (inset === true) {
    return `inset ${x}px ${y}px ${blurRad}px ${color}`
  } else if (spreadRad === 0) {
...
// FUNC: textShadowCSS()
// Text-Shadow CSS
function textShadowCSS(x, y, blurRad = 0, col = color(0)) {
  return boxShadowCSS(x, y, blurRad, col)
}

//TODO: DEPRECATE
// FUNC: dropShadowCSS()
// Drop-Shadow CSS
function dropShadowCSS(x, y, blurRad = 0, col = color(0)) {
  return `drop-shadow(${boxShadowCSS(x, y, blurRad, col)})`
}

//TODO: DEPRECATE
// FUNC: maxComponent()
function maxComponent(vector = Shade.shadVect()) {
  return vector.y
  // return max(this.x, this.y)
}

//TODO: DEPRECATE
// FUNC: neuBoxShadowCSS()
//Neumorphic Box-Shadow CSS - create highlight shadow pair code for CSS
function neuBoxShadowCSS(vector = Shade.shadVect(), blurRad, highCol, shadCol, inset = false) {
  let highlightCSS = boxShadowCSS(-vector.x, -vector.y, blurRad, 0, highCol, inset)
  let shadowCSS = boxShadowCSS(vector.x, vector.y, blurRad, 0, shadCol, inset)
  return `${shadowCSS}, ${highlightCSS}`
  // return join([shadowCSS, highlightCSS], CS.comma)
}

//TODO: DEPRECATE
// FUNC: neuBoxShadowFactory()
// Neumorphic Box Shadow Factory - create a shadow and highlight stack
function neuBoxShadowFactory(baseCol = GRAY, vector = Shade.shadVect(), start = 0.5, spread = 16, inset = false) {
  let offset = vector.mag() / sqrt(2)
  // print(`offset: ${offset}`)
  let cols = hiShSpreadfromCol(spread, baseCol)
  let neuShads = createSlices(start, offset, globalControls.shadQuality)
  // print(neuShads.map(e => e.toFixed(2))) 
  neuShads = neuShads.map(sliceOffset => neuBoxShadowCSS(vector.setMag(sliceOffset), 2 * sliceOffset, cols[0], cols[1], inset))
  return neuShads
}

//TODO: DEPRECATE
// FUNC: neuDropShadowFactory()
function neuDropShadowFactory(baseCol = GRAY, vector = Shade.shadVect(), start = 0.5, spread = 16) {
  let offset = vector.mag() / sqrt(2)
  // print(`offset: ${offset}`)
  let cols = hiShSpreadfromCol(spread, baseCol)
  let neuShads = createSlices(start, offset, globalControls.shadQuality)
...
//TODO: DEPRECATE
// FUNC: hiShSpreadfromCol()
// Highlight and Shadow colors spread from base color
function hiShSpreadfromCol(spread = 16, baseCol = color(127)) {
  let b = brightness(baseCol)
  let bPair = [round(b + spread), round(b - 2 * spread)]
  let cols = bPair
    .map(b => `hsb(${hue(baseCol)}, ${saturation(baseCol)}%, ${b}%)`)
    .map(dscrpt => color(dscrpt))
  return cols
}

//TODO: DEPRECATE
// FUNC: complimentHighShadSpread()
function complimentHighShadSpread(spread = 16, baseCol = color(127)) {
  let hueVal = round(hue(baseCol))
  let satVal = saturation(baseCol)
  let briVal = brightness(baseCol)

  let highCol = [hueVal, satVal, constrain(briVal + spread, 0, 100)]
  let shadCol = [complimentVal(hueVal), satVal, constrain(briVal - 2 * spread, 0, 100)]

  let cols = [highCol, shadCol]
    .map(hsb => `hsb(${hsb[0]}, ${hsb[1]}%, ${hsb[1]}%)`)
    .map(dscrpt => color(dscrpt))
  return cols
}
...
//TODO: DEPRECATE
// FUNC: complimentVal()
function complimentVal(hue = 180) {
  return (round(hue + 180) % 360)
}
//TODO: DEPRECATE
// FUNC: randomHighHue()
function randomHighHue() {
  let hue = floor(R.random_num(0, 255))
  return color(`hsb(${hue}, 100%, 100%)`)
}
//TODO: DEPRECATE
// FUNC: randomShadHue()
function randomShadHue() {
  let hue = floor(R.random_num(0, 255))
  return color(`hsb(${hue}, 100%, 50%)`)
}

// slice exp series
// function sliceExpSeries(min, max) {
//   let startIndex = expSeries.findIndex(e => e >= min)
//   let endIndex = expSeries.findIndex(e => e >= max)
//   let maxInterval = expSeries.slice(endIndex - 1, endIndex + 1)
//   let scalar = convertRange(max, maxInterval, [0.5, 1])
//   let scaledLastValue = maxInterval[1] * scalar
//   let series = expSeries.slice(startIndex, endIndex)
//   series.push(scaledLastValue)
