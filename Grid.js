...
class Grid extends ProtoLayer {
  ...
  completeEnds = (testPool, wrap = true) => {
    ...
    // testPool = testPool.slice(0, 10)                                       //TESTING: reduce processed
    testPool.forEach(s => {
  ...
  maximizeCuddles(
    ...
    const wrapInnerMost = (testPool = defaultPool, preserveQuads = preserveQs, balanced = balance) => {
      ...
      // testPool = testPool.slice(0, 4)                                        //TESTING: reduce processed
      testPool.forEach(s => {
  ...
  outline({
    ...
    while (amount > 0 && !this.isFull) {
      if (selection?.length > 0) {
        // DeBug.log(`outline direction`, direction)
  ...
  seed(name, options) {
    ...
    switch (name) {
      ...
      case 'Empty':
        break
      default: DeBug.error(`Grid.seed: ${name} not found`)
    }
  }
  // #endregion
  ...
  assignElement() {
    super.assignElement()
    this.svgElt.attribute('overflow', 'visible')  // § 9.14 — let filter effects extend beyond grid inset viewport; FRAME <svg> provides outer clip
