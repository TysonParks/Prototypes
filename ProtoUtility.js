...
// TODO: implement Shape and Puzzle
...
// MARK: DEPRECATE
// #region DEPECATE
// TODO: This might be completely unnecessary because I can and probably should just assign callbacks externally using css selectors after all the layout creation magic has completed and we have a remaining set of ProtoLayers - CSS FTW!
// CLASS: StateObserver
// NOTE: https://stackoverflow.com/a/60701062, https://jsfiddle.net/leviwheatcroft/wqf9jL2g/42/
// Instead of 3 classes (1 super and two subs) I put both Observed and Reactive methods into the same class because I know each ProtoLayer will need to be able to do both
class StateObserver {
  instances
  state
  lastState
  ...
  stateUpdate(update) {
    ...
    StateObserver.instances.forEach(i => {
      if (!i.onStateUpdate) {
        return i.onStateUpdate(update, StateObserver.lastState)
      }
    })
  }

  connectedCallback(eltID = this.id, trigger) {
    select(elementID).mousePressed(trigger)
  }

  onStateUpdate(state, lastState, reaction = this.reaction) {
    if (state.reaction === lastState.reaction) {
      reaction
    }
  }
